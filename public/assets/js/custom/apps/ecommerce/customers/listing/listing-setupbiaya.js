...
function editsetup(id){
    ...
    $.ajax({
        url:'getsetupbiayapinjaman',
        type:'POST',
        data:{id:id},
        success:function(response){
            var data = JSON.parse(response);
            $('#jenis_pinjaman').val(data.message.id_pinjaman);
            $('#nama_biaya').val(data.message.nama_biaya);
            $('#jenis_biaya').val(data.message.jenis_biaya);
            $('#jumlah').val(data.message.jumlah);
