...
function editsetup(id){
    ...
    $.ajax({
        ...
        success:function(response){
            ...
            $('#jenis_pinjaman').val(data.message.jenis_pinjaman);
...
function deletesetupsimpanan(id){
    Swal.fire({
        ...
        if(e.value){
            $.ajax({
                url:'deletesetuppinjaman',
                type:'post',
                data:{id:id},
                success:function(response){
                    location.href='/setuppinjaman';
                }
            });
        }
    });
...
function pilihanggota(id){
    ...
    $('#kt_modal_add_customer').modal('show');
    $.ajax({
        url:'getdataanggotabykode',
        type:'get',
        data:{id:id},
        success:function(response){
            var data = JSON.parse(response);
            $('#kode_anggota').val(data.message.no_anggota);
            $('#nama').val(data.message.nama);
            $('#alamat').val(data.message.alamat);
            $('#no_telp').val(data.message.no_telp);
        }
    });
    $('#kt_modal_add_customer').modal('hide');
}

function changesimpanan(id){
    $.ajax({
        ...
        success:function(response){
            var data = JSON.parse(response);
            $('#jangka_waktu').val(data.message.jangka);
            $('#bunga').val(data.message.bunga);
            
        }
    });
}

function changepengajuan(d){
    if(d==0){
        $('.anggota').hide();
        $('.bukan-anggota').show();
    }else{
...
function changepinjaman(id){
    location.href='pengajuanpinjaman?id='+id;
}

function changepinjamanawal(id){
    location.href='pinjamanawal?id='+id;
}
